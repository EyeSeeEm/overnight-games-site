...
function checkCollisions() {
  ...
  if (currentRoom && currentRoom.cleared) {
    for (const door of currentRoom.doors) {
      ...
      // Use >= and <= to allow player to enter doors at boundaries
      // (Player y is clamped to UI_HEIGHT+20, so need <= for north door at UI_HEIGHT+20)
...
// ═══════════════════════════════════════════════════════════════════════════
// V2 TEST HARNESS - Time-Accelerated Execution
// ═══════════════════════════════════════════════════════════════════════════
...
(function() {
  ...
  function logEvent(msg) {
    debugLogs.push(`[${totalGameTime}ms] ${msg}`);
  }

  // Hook into game events - we'll patch key functions to log events
  const originalCheckCollisions = window.checkCollisions || checkCollisions;

  window.harness = {
    /**
     * MAIN METHOD - Execute an action with time acceleration.
     */
    execute: async function({ keys: keyList = [], duration = 500, screenshot = false }) {
      const startReal = performance.now();
      debugLogs = []; // Clear logs for this execution

      // Set active keys
      activeKeys = new Set(keyList);

      // Map key names to key codes for the game's input system
      const keyCodeMap = {
        'w': 'KeyW', 'W': 'KeyW',
        'a': 'KeyA', 'A': 'KeyA',
        's': 'KeyS', 'S': 'KeyS',
      ...
      const startEnemyCount = enemies.length;
      const startHP = player ? player.hp : 0;
      ...
      for (let i = 0; i < ticks; i++) {
        ...
        if (player) {
          if (player.hp < startHP && player.hp > 0) {
            logEvent(`Player took damage: ${startHP} -> ${player.hp}`);
          }
        }
      }

      // Log enemy kills
      const endEnemyCount = enemies.filter(e => e.active).length;
      if (endEnemyCount < startEnemyCount) {
        logEvent(`Enemies killed: ${startEnemyCount - endEnemyCount}`);
      }
    ...
    /**
     * Get current state without executing any time.
     */
    getState: function() {
      return {
        gameState: gameState,
        ...
        room: currentRoom ? {
          ...
          key: floorMap ? floorMap.currentRoomKey : null
        } : null,
        floor: currentFloor,
        roomsCleared: roomsCleared,
     ...
     */
    getPhase: function() {
      return gameState; // 'menu', 'playing', 'paused', 'gameover', 'victory'
    },
    ...
    debug: {
      ...
      setHealth: function(hp) {
        if (player) player.hp = Math.min(player.maxHP, Math.max(0, hp));
      },
      ...
      giveAmmo: function(n) {
        if (player) player.ammo = Math.min(player.maxAmmo, player.ammo + n);
      },
      giveBombs: function(n) {
        if (player) player.bombs = Math.min(player.maxBombs, player.bombs + n);
      },
      setWeapon: function(name) {
        if (player && WEAPONS[name]) {
          player.currentWeapon = { ...WEAPONS[name] };
          player.ammo = player.currentWeapon.maxAmmo === Infinity ? 100 : player.currentWeapon.maxAmmo;
        }
      },
      skipToBoss: function() {
  ...
  // Helper: Check if key is active (for external use)
  window.isKeyActive = function(key) {
