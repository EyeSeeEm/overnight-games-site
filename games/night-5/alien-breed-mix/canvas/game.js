...
let corridors = []; // Corridors connecting rooms
...
function generateLevel(deck) {
    ...
    // Generate room layout with corridor spacing
    const roomGrid = [];
    const roomWidth = 8;  // Slightly smaller rooms to make space for corridors
    const roomHeight = 6;
    const corridorWidth = 3; // Corridor width in tiles
    const corridorLength = 4; // Corridor length in tiles
    const gridCols = Math.floor(levelWidth / (roomWidth + corridorLength));
    const gridRows = Math.floor(levelHeight / (roomHeight + corridorLength));

    // Create rooms with spacing for corridors
    for (let gy = 0; gy < gridRows; gy++) {
        roomGrid[gy] = [];
        for (let gx = 0; gx < gridCols; gx++) {
            // Position includes corridor spacing
            const room = {
                gx, gy,
                x: gx * (roomWidth + corridorLength) * TILE_SIZE,
                y: gy * (roomHeight + corridorLength) * TILE_SIZE,
    ...
    // Create corridors and doors between rooms
    for (let gy = 0; gy < gridRows; gy++) {
        for (let gx = 0; gx < gridCols; gx++) {
            const room = roomGrid[gy][gx];

            // Corridor to right (horizontal corridor)
            if (gx < gridCols - 1) {
                const nextRoom = roomGrid[gy][gx + 1];

                // Corridor connects right side of room to left side of next room
                const corridorX = room.x + room.width;
                const corridorY = room.y + room.height / 2 - (corridorWidth * TILE_SIZE / 2);
                const corridorW = corridorLength * TILE_SIZE;
                const corridorH = corridorWidth * TILE_SIZE;
                ...
                // Add corridor walls (top and bottom)
                walls.push({
                    x: corridorX,
                    y: corridorY - TILE_SIZE,
                    w: corridorW,
                    h: TILE_SIZE
                });
                walls.push({
                ...
                const doorX = corridorX - TILE_SIZE;
                const doorY = corridorY;
                doors.push({
                    x: doorX,
                    y: doorY,
                    w: TILE_SIZE * 2,
                    h: corridorWidth * TILE_SIZE,
                    direction: 'horizontal',
                    open: false,
            ...
            // Corridor to bottom (vertical corridor)
            if (gy < gridRows - 1) {
                const nextRoom = roomGrid[gy + 1][gx];

                // Corridor connects bottom of room to top of next room
                const corridorX = room.x + room.width / 2 - (corridorWidth * TILE_SIZE / 2);
                const corridorY = room.y + room.height;
                const corridorW = corridorWidth * TILE_SIZE;
                const corridorH = corridorLength * TILE_SIZE;
                ...
                // Add corridor walls (left and right)
                walls.push({
                    x: corridorX - TILE_SIZE,
                    y: corridorY,
                    w: TILE_SIZE,
                    h: corridorH
                });
                ...
                // Door at start of corridor
                const doorX = corridorX;
                const doorY = corridorY - TILE_SIZE;
                doors.push({
                    x: doorX,
                    y: doorY,
                    w: corridorWidth * TILE_SIZE,
                    h: TILE_SIZE * 2,
                    direction: 'vertical',
                    open: false,
                    keycardRequired: nextRoom.keycardRequired,
    ...
    // Remove wall segments where doors and corridors are
    ...
    // Remove walls where corridors connect to rooms
    for (const corridor of corridors) {
        walls = walls.filter(wall => {
            // Check if wall blocks corridor entrance/exit
            const overlapsX = wall.x < corridor.x + corridor.width && wall.x + wall.w > corridor.x;
            const overlapsY = wall.y < corridor.y + corridor.height && wall.y + wall.h > corridor.y;
