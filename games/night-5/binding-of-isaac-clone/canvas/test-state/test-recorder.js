...
window.testRecorder = {
   ...
   * Uses window.debug.getState() if available, otherwise captures what we can
   */
  _captureState: function() {
    if (window.debug && typeof window.debug.getState === 'function') {
      try {
        return window.debug.getState();
      } catch (e) {
    ...
    return {
      timestamp: Date.now(),
      note: 'window.debug.getState() not available'
    };
  }
};
...
window.testAPI = {
  ...
  doAction: (intent, action, waitMs = 500) => {
    return new Promise(resolve => {
      ...
      if (action && window.debug) {
        if (action.type === 'moveDir' && window.debug.movePlayer) {
          window.debug.movePlayer(action.direction, action.duration || waitMs);
        } else if (action.type === 'fire' && window.debug.fire) {
          window.debug.fire(action.direction);
        } else if (action.type === 'debug' && window.debug[action.command]) {
          window.debug[action.command](...(action.args || []));
        }
      }
...
console.log('[TestRecorder] Loaded. Use testRecorder.start() to begin recording.');
