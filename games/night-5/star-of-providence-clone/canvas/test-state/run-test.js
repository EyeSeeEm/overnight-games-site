 ...
 * Game: Star of Providence Clone (Canvas)
 *
 * Usage:
 ...
 *   node run-test.js --type feature_test --target "boss-chamberlord" --iteration 3
 */
...
async function runTest() {
  ...
  try {
    ...
    const hasDebug = await page.evaluate(() => !!window.debugCommands);

    // Start the game if on title screen
    const gameState = await page.evaluate(() => window.debugCommands?.getState?.()?.gameState);
    if (gameState === 'title') {
 ...
 * Star of Providence controls: WASD/Arrows, Space (fire), Shift (focus), Z (dash), X (bomb)
 */
async function runGeneralPlaytest(page, duration, takeScreenshot) {
  ...
  const actionKeys = [' ', 'z']; // fire, dash
  let actionCount = 0;

  while (Date.now() - startTime < duration) {
    // Determine intent based on action type
    const doFire = actionCount % 3 === 0;
    const doDash = actionCount % 7 === 0;

    let intent, actionKey, actionName;
    ...
    if (doFire) {
      ...
      intent = 'Dash to avoid bullets';
      actionKey = 'z';
      actionName = 'dash';
    } else {
      intent = 'Move to explore room';
      actionKey = moveKeys[actionCount % 4];
      actionName = `move_${actionKey}`;
    }

    // Execute action with hold duration
    const holdDuration = actionKey === ' ' ? 200 : actionKey === 'z' ? 100 : 600 + Math.random() * 300;
    ...
    await page.evaluate(({i, a, d}) => {
      const state = window.debugCommands ? window.debugCommands.getState() : { note: 'no debug' };
...
/**
 * Feature test - test a specific feature
 */
